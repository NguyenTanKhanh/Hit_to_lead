...
def calc_logS_esol(m: Chem.Mol) -> float:
    """
    ESOL (Delaney-style) aqueous solubility estimate: logS (mol/L).
    Common implementation:
    logS = 0.16 - 1.5*logP - 0.01*(MW - 40) + 0.066*RB + 0.066*AP
    AP = aromatic proportion = (# aromatic heavy atoms) / (heavy atoms)
    """
    ...
    arom_heavy = sum(1 for a in m.GetAtoms() if a.GetIsAromatic() and a.GetAtomicNum() > 1)
    ap = (arom_heavy / heavy) if heavy else 0.0

    logS = 0.16 - 1.5*logp - 0.01*(mw - 40.0) + 0.066*rb + 0.066*ap
    return float(logS)
