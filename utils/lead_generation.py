...
def normalize_frag(smi: str) -> str:
    """Convert any SMILES input into RDKit canonical form without stereo."""
    mol = Chem.MolFromSmiles(smi)
    if mol is None:
        raise ValueError(f"Cannot parse fragment SMILES: {smi}")

    # Convert SMILES to canonical form without stereochemistry (/ and \ removed)
    smi_norm = Chem.MolToSmiles(
        mol,
        canonical=True,
...
def generate_molecules(model_path, frags, num_samples, output_dir, gpu_id=0, min_sa=0.9):
    ...
    # Normalize all fragment inputs BEFORE sending to DrugEx
    frags = [normalize_frag(f) for f in frags]
    ...
    smiles = [
        replace_iodine_with_hydrogen(s)
