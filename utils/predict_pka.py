...
def _predict_single_ckpt(ckpt_path, dataset, num_workers=0):
    ...
    trainer = pl.Trainer(
        ...
        devices=1,
        strategy="auto",   # Avoid MPI/Slurm detection
    )
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Filter SMILES by regression model (pKa range)."
    )
    ...
    ap.add_argument(
        "--out-smi",
        required=True,
        help="Output SMILES file containing molecules that passed the filter",
    )
    ap.add_argument(
        "--reg-ckpt",
        required=True,
        help="Regression model checkpoint",
    )
    ap.add_argument(
        "--reg-min",
        type=float,
        default=None,
        help="Minimum cutoff (prediction >= value)",
    )
    ap.add_argument(
        "--reg-max",
        type=float,
        default=None,
        help="Maximum cutoff (prediction <= value)",
    )
    ap.add_argument(
        "--num-workers",
        type=int,
